"""V2 Profile Memory Extractor - Explicit info + Implicit traits extractor.
...
class ProfileMemoryV2Extractor(MemoryExtractor):
    """V2 Profile Extractor - Supports explicit info + implicit traits."""
    ...
    async def extract_memory(
        self, request: ProfileMemoryV2ExtractRequest
    ) -> Optional[ProfileMemoryV2]:
        """Extract V2 Profile from 3 types of information.
        ...
        if not new_episode:
            logger.warning("No new episode provided for V2 profile extraction")
            return old_profile
        
        # Initialize profile
        if old_profile is None:
            logger.info(f"[V2Extractor] No old_profile for user={request.user_id}, creating new")
            current_profile = ProfileMemoryV2(
            ...
            logger.info(f"[V2Extractor] Using old_profile for user={request.user_id}: "
        ...
        logger.info(
            f"Processing V2 profile: cluster={len(cluster_episodes)}, new=1"
        )
        ...
        if current_profile.total_items() > compact_threshold:
            logger.info(f"Profile has {current_profile.total_items()} items (threshold={compact_threshold}), compacting to {compact_target}...")
            current_profile = await self._compact_profile(current_profile, compact_target)
    ...
    async def _llm_update_profile(
        ...
        # Get prompt template
        prompt_template = get_prompt_by("PROFILE_V2_UPDATE_PROMPT")
        prompt = prompt_template.format(
            current_profile=profile_text if profile_text else "(Empty, no records yet)",
            conversations=conversations_text if conversations_text else "(No conversations)",
        )
        ...
        try:
            ...
            implicit_list = [trait.to_dict() for trait in current_profile.implicit_traits]
            
            # Build timestamp mapping
            id_to_ts = self._build_timestamp_map(current_profile, cluster_episodes, new_episode)
            ...
            for op in operations:
                ...
                if action == "none":
                    ...
                    data["sources"] = [self._attach_ts(s, id_to_ts) for s in data.get("sources", [])]
                    ...
                    data = op.get("data", {})
                    target_list = explicit_list if op_type == "explicit_info" else implicit_list
    ...
    async def _compact_profile(
        self, profile: ProfileMemoryV2, max_items: int
    ) -> ProfileMemoryV2:
        ...
        # Get prompt template
        prompt_template = get_prompt_by("PROFILE_V2_COMPACT_PROMPT")
        prompt = prompt_template.format(
            total_items=total,
            max_items=max_items,
    ...
    def _merge_missing_items(
        self, old_profile: ProfileMemoryV2, new_dict: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for info in old_profile.explicit_info:
            ...
            if desc_lower and desc_lower not in new_explicit_descs:
                ...
                logger.warning(f"[Profile] Recovered dropped explicit_info: {info.description[:50]}...")
        ...
        for trait in old_profile.implicit_traits:
            ...
            if desc_lower and desc_lower not in new_implicit_descs:
                ...
                logger.warning(f"[Profile] Recovered dropped implicit_trait: {trait.trait_name}...")
        
        # Merge back dropped items
        if dropped_explicit:
            new_dict["explicit_info"] = (new_dict.get("explicit_info") or []) + dropped_explicit
            logger.info(f"[Profile] Merged back {len(dropped_explicit)} dropped explicit_info items")
        
        if dropped_implicit:
            new_dict["implicit_traits"] = (new_dict.get("implicit_traits") or []) + dropped_implicit
            logger.info(f"[Profile] Merged back {len(dropped_implicit)} dropped implicit_traits items")
